      ...
      <div className="relative mb-8 rounded-2xl overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 p-8">
          ...
          {opportunity.nonprofit && (
            <h2 className="text-2xl font-semibold text-white/90 drop-shadow-lg">
              {opportunity.nonprofit}
            </h2>
          )}
          {opportunity.host_org_name && (
            <h3 className="text-xl font-semibold text-white/80 drop-shadow-lg">
              Hosted by {opportunity.host_org_name}
            </h3>
          )}
          {isUserHost && (
            <div className="mt-2">
              <span className="text-white bg-green-600/80 px-3 py-1 rounded-full text-sm font-semibold">
                You are the host
              </span>
            </div>
          )}
          {currentUser.admin && !isUserHost && (
            <div className="mt-2">
              <span className="text-white bg-blue-600/80 px-3 py-1 rounded-full text-sm font-semibold">
                Admin View
              </span>
            </div>
          )}
                      ...
                      {userLookupResults.length > 0 ? (
                        <>
                          <p className="text-sm text-gray-600 font-medium">
                            {userLookupResults.length} user
                            {userLookupResults.length !== 1 ? 's' : ''} found:
                          </p>
                          {userLookupResults.map((user) => {
                            const isAlreadyRegistered = signedUpStudents.some(
                              (s) => s.id === user.id
                            );
                            return (
                              <div
                                key={user.id}
                                className="p-3 bg-white border border-purple-200 rounded-lg hover:bg-purple-50 transition-colors"
                              >
          ...
          {canManageOpportunity && availableSlots <= 0 && (
            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-700 text-center">
                ⚠️ This event has reached its maximum capacity. Consider increasing the slot limit
      ...
      <div className="lg:col-span-1 space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p
                      className={`text-lg font-semibold ${availableSlots > 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {availableSlots}
                    </p>
                  </div>
                ...
                <button
                  onClick={() => {
                    ...
                    if (participantEmails) {
                      ...
                      alert(
                        'No participant emails available to send to. Please check the console for debugging information.'
                      );
                    }
                  }}
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg">
            <ul className="space-y-4 text-gray-700">
              <li className="flex items-center gap-3">
                <CalendarIcon /> <span>{displayDate}</span>
              </li>
              <li className="flex items-center gap-3">
                <ClockIcon />{' '}
                <span>
              ...
              <li className="flex items-center gap-3">
                <UsersIcon />{' '}
                <span>
                  {availableSlots} of {opportunity.total_slots} slots remaining
                </span>
              </li>
              <li className="flex items-center gap-3">
                <StarIcon /> <span>{opportunity.points} points</span>
              </li>
            </ul>
            <button
            ...
            {!isUserSignedUp && availableSlots <= 0 && (
              <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-sm text-red-700 text-center">
                  This event has reached its maximum capacity of {opportunity.total_slots}{' '}
                  participants.
                </p>
              </div>
            )}
        ...
        (opportunity.tags && opportunity.tags.length > 0) ? (
          <div className="bg-white p-6 rounded-2xl shadow-lg">
            ...
            <div className="space-y-3">
              {opportunity.causes &&
                Array.isArray(opportunity.causes) &&
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {opportunity.causes.map((cause, index) => (
                        <span
                          key={index}
                          className="bg-cornell-red/10 text-cornell-red px-3 py-1 rounded-full text-sm font-medium"
                        >
                          {cause}
                        </span>
                      ))}
        ...
        {opportunity.host_id && (
          <div className="bg-white p-6 rounded-2xl shadow-lg">
            ...
            {(() => {
                ...
                return (
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-gray-800 hover:text-cornell-red transition-colors">
                          {hostUser.name}
                        </p>
                    ...
                    {opportunity.host_org_name && (
                      <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="h-5 w-5 text-cornell-red"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
              ...
              return (
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-500">Host information not available.</p>
                </div>
              );
            })()}
          </div>
...
const CalendarIcon = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    className="h-6 w-6 text-cornell-red"
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
  >
...
const UsersIcon = () => <img src="/icons/groups-icon.png" alt="Groups" className="h-6 w-6" />;
const StarIcon = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    className="h-6 w-6 text-cornell-red"
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M12 18v-5.5M12 8V6m0 14h.01M7 12h.01M17 12h.01M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42M5.64 5.64L4.22 4.22m14.14 0l-1.42 1.42"
    />
  </svg>
);
