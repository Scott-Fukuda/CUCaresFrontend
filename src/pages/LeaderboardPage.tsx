...
const LeaderboardPage: React.FC<LeaderboardPageProps> = ({
    ...
    const sortedOrgs = filteredOrgs
      .map((org) => {
        ...
        const totalPoints = memberIds.reduce(
          (sum, memberId) => sum + (userPointsMap.get(memberId) || 0),
          0
        );
  ...
  const UserRow = ({ user, points, rank }: { user: User; points: number; rank: number }) => {
    ...
    return (
      <li
        className={`flex items-center justify-between py-4 ${isCurrentUser ? 'bg-yellow-50 rounded-lg -mx-4 px-4' : ''}`}
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <span className="font-bold text-lg text-gray-500 w-6 text-center flex-shrink-0">
            {rank}
          </span>
          <img
          ...
          />
          <span
            className="font-medium text-gray-900 cursor-pointer truncate min-w-0"
            onClick={() => navigate(`/profile/${user.id}`)}
          >
            {user.name}
          </span>
          ...
          {!isCurrentUser &&
            (isFriend ? (
              ...
              <button
                ...
                className="text-xs bg-gray-200 text-gray-700 font-medium py-1 px-2 rounded-full hover:bg-gray-300 transition-colors text-center min-w-[70px]"
              >
                Add Friend
              </button>
            ))}
              ...
              return (
                <li key={org.id} className="flex items-center justify-between py-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    >
                      <p className="font-medium text-gray-900 hover:text-cornell-red truncate">
                        {org.name}
                      </p>
                      <p className="text-sm text-gray-500">
                        {memberCount} members &bull; {org.type}
                      </p>
                    </div>
                  </div>
