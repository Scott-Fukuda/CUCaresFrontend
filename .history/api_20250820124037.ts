...
export const getOpportunities = async (): Promise<Opportunity[]> => {
    try {
        ...
        const transformedOpportunities = (response.opportunities || []).map((opp: any) => {
            ...
            console.log(`Opportunity ${opp.id} date parsing:`, {
                originalDate: opp.date,
                parsedDateObj: dateObj.toISOString(),
                dateOnly,
                timeOnly,
                gmtHours: opp.date.includes('GMT') ? dateObj.getUTCHours() : null,
                easternHours: opp.date.includes('GMT') ? (dateObj.getUTCHours() - 4 + 24) % 24 : null
            });
            ...
            // Build image URL from local static path if a filename is provided by backend
            const rawImageRef = opp.image || opp.image_url || opp.imageUrl || '';
            const imageFileName = typeof rawImageRef === 'string' && rawImageRef
              ? (rawImageRef.split('/').pop() as string)
              : '';
            const resolvedImageUrl = imageFileName
              ? `${ENDPOINT_URL}/static/images/opportunities/${imageFileName}`
              : (opp.image_url || 'https://via.placeholder.com/300x200?text=Opportunity');
