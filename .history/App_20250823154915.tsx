...
export type Page = 'opportunities' | 'leaderboard' | 'profile' | 'groups' | 'notifications' | 'opportunityDetail' | 'groupDetail' | 'createOpportunity';
...
const App: React.FC = () => {
  ...
  const currentUserSignupsSet = useMemo(() => {
    ...
    // Don't automatically include host opportunities - let backend data drive this
    return userSignups;
  }, [currentUser, signups]);
  ...
  const getUserFriends = useMemo(() => {
    if (!currentUser) return [];
    // This will be updated to use the new API endpoint
    // For now, we'll use the existing logic but it should be replaced with API call
    return friendships
      .filter(f => f.status === 'accepted' && (f.user1_id === currentUser.id || f.user2_id === currentUser.id))
      .map(f => {
        const friendId = f.user1_id === currentUser.id ? f.user2_id : f.user1_id;
        return students.find(s => s.id === friendId);
      })
      .filter((user): user is User => user !== undefined);
  }, [friendships, currentUser, students]);
  ...
  const getFriendsForUser = useCallback(async (userId: number): Promise<User[]> => {
    ...
    try {
      const friendsResponse = await api.getUserFriends(userId);
      console.log('Friends response from API:', friendsResponse);
      
      // The API returns { count: number, friendships: [...] }
      if (friendsResponse && Array.isArray(friendsResponse.friendships)) {
        const acceptedFriendships = friendsResponse.friendships.filter((f: any) => f.accepted === true);
        console.log('Accepted friendships:', acceptedFriendships);
        
        // For each accepted friendship, we need to find the other user
        // Since we don't have the other user's ID directly, we'll need to match by name
        // This is a temporary solution - ideally the backend should return user IDs
        const friendUsers: User[] = [];
        
        for (const friendship of acceptedFriendships) {
          // Try to find the user by name (this is not ideal but works with current backend)
          const friendUser = students.find(s => 
            `${s.firstName} ${s.lastName}` === friendship.requester_name ||
            `${s.firstName} ${s.lastName}` === friendship.receiver_name
          );
          
          if (friendUser && friendUser.id !== userId) {
            console.log(`Found friend: ${friendUser.firstName} ${friendUser.lastName}`);
            friendUsers.push(friendUser);
          }
        }
        
        console.log('Final friends list:', friendUsers.map(u => `${u.firstName} ${u.lastName}`));
        return friendUsers;
      }
  ...
  const handleRequestResponse = (fromUserId: number, response: 'accepted' | 'declined') => {
    ...
    if (response === 'accepted') {
      ...
      if (friendship) {
        handleAcceptFriendRequest(friendship.id);
      }
    } else {
      // Find the friendship ID for this request
      const friendship = friendships.find(f => 
      ...
      if (friendship) {
        handleRejectFriendRequest(friendship.id);
      }
    }
  };
  ...
  const renderPage = () => {
    ...
    switch (pageState.page) {
        ...
        case 'createOpportunity':
            return <CreateOpportunityPage currentUser={currentUser} allOrgs={organizations} setPageState={setPageState} />;
        default:
            return <OpportunitiesPage opportunities={opportunities} students={students} allOrgs={organizations} signups={signups} currentUser={currentUser} handleSignUp={handleSignUp} handleUnSignUp={handleUnSignUp} setPageState={setPageState} currentUserSignupsSet={currentUserSignupsSet} />;
    }
  };
