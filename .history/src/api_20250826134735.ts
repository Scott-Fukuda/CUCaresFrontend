...
import { User, Opportunity, Organization, SignUp, Friendship, FriendRequest, FriendshipStatus } from './types';
...
export const setProfilePictureUrl = (profilePictureUrl?: string | null, userId?: number): string => {
  const s3ObjectUrl = (process.env.VITE_S3_OBJECT_URL ?? '');
  // Object URL looks like: https://your-s3-bucket.s3.us-east-1.amazonaws.com/images/image-hash
};
...
export const getUsers = async (): Promise<User[]> => {
    ...
    return response.users.map((u: any) => ({
        ...
        organizationIds: (u.organizations || []).map((org: any) => org.id) || [],
        friendIds: u.friendIds || [],
        interests: u.interests || [],
        // Use helper function for profile picture
        profilePictureUrl: getProfilePictureUrl(u.profilePictureUrl || u.profile_image, u.id),
        // Handle registration_date field from backend
        registration_date: u.registration_date || u.registrationDate,
    }));
};
...
export const getUnapprovedOrgs = async (): Promise<Organization[]> => {
    const response = await request('/orgs/unapproved');
    return response.organizations || [];
};
...
export const updateOrganization = (id: number, data: object): Promise<Organization> => request(`/orgs/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data)
});
export const deleteOrganization = (id: number): Promise<void> => request(`/orgs/${id}`, {
    method: 'DELETE'
});
...
export const getAllFriendships = async (): Promise<Friendship[]> => {
  try {
    return await request('/friendships');
  } catch (error) {
    console.warn(`API: Friendships endpoint not available yet: ${error}`);
    return [];
  }
};
...
export const getUserFriends = async (userId: number): Promise<any> => {
  ...
  // Handle case where user has no friendships (empty array or null)
  if (result && (result.friendships === null || result.friendships === undefined || (Array.isArray(result.friendships) && result.friendships.length === 0))) {
...
export const getUserFriendRequests = async (userId: number): Promise<FriendRequest[]> => {
  ...
  const result = await request(`/users/${userId}/friend-requests`);
...
export const getUserFriendships = async (userId: number): Promise<Friendship[]> => {
  ...
  try {
  const result = await request(`/users/${userId}/friendships`);
  ...
  return rawFriendships.map(friendship => ({
    ...
    console.warn(`API: User friendships endpoint not available yet: ${error}`);
    return [];
  }
};

export const sendFriendRequest = (userId: number, friendId: number) => request(`/users/${userId}/friends`, {
  method: 'POST',
  body: JSON.stringify({ receiver_id: friendId }),
});

export const acceptFriendRequest = (friendshipId: number, receiverId: number) => request(`/friendships/${friendshipId}/accept`, {
  method: 'PUT',
  body: JSON.stringify({ receiver_id: receiverId }),
});

export const rejectFriendRequest = (friendshipId: number, receiverId: number) => request(`/friendships/${friendshipId}/reject`, {
  method: 'PUT',
  body: JSON.stringify({ receiver_id: receiverId }),
});
...
export const checkFriendshipStatus = async (userId: number, friendId: number): Promise<FriendshipStatus> => {
  ...
  try {
    ...
    console.warn(`API: Friendship status endpoint not available yet: ${error}`);
    // Return a default status when endpoint is not available
    return { status: 'none' };
  }
};
...
export const getAcceptedFriendships = async (userId: number): Promise<User[]> => {
  ...
  try {
    ...
    if (Array.isArray(result)) {
      ...
      friendsArray = result.friendships;
    } else if (result && Array.isArray(result.data)) {
    ...
    // If we found a valid array (even if empty), process it
    if (friendsArray.length >= 0) {
      // Transform the friendship objects to User objects
    ...
    // Return empty array when endpoint is not available
    return [];
  }
};
...
export const getOpportunities = async (): Promise<Opportunity[]> => {
    try {
        ...
        const transformedOpportunities = (response.opportunities || []).map((opp: any) => {
            ...
            const resolvedImageUrl = opp.image_url || opp.image || opp.imageUrl || 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 200" fill="%23E5E7EB"%3E%3Crect width="300" height="200"/%3E%3Ctext x="150" y="100" text-anchor="middle" fill="%236B7280" font-family="Arial" font-size="16"%3EOpportunity Image%3C/text%3E%3C/svg%3E';
