  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        try {
          const imageUrl = await uploadProfilePicture(selectedImage);
        ...
        prevOpportunities.map(opp => 
          opp.id === opportunity.id 
            ? {
                ...
                time: editForm.time,
                duration: editForm.duration,
                imageUrl: updateData.image || opp.imageUrl // Update image URL if new image was uploaded
              }
            : opp
        )
      );
      ...
      Object.assign(opportunity, {
        ...
        time: editForm.time,
        duration: editForm.duration,
        imageUrl: updateData.image || opportunity.imageUrl // Update image URL if new image was uploaded
      });
                        ...
                        <div className="flex gap-2">
                            {!isEditing ? (
                                ...
                                <>
                                    ...
                                    >
                                        {isSaving ? (isUploadingImage ? 'Uploading Image...' : 'Saving...') : 'Save Changes'}
                                    </button>
                                    <button
                            ...
                            <div className="space-y-3">
                                ...
                                <div>
                                    <input
                                        ...
                                        className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-cornell-red file:text-white hover:file:bg-red-800 file:cursor-pointer cursor-pointer"
                                    />
                                    <p className="text-xs text-gray-500 mt-1">
                                        Select a new image to replace the current photo. Leave empty to keep current photo.
                                    </p>
                                </div>
                            </div>
                        </div>
