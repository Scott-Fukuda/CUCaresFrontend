...
import { getProfilePictureUrl, updateOpportunity } from '../api';
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            {signedUpStudents.map(student => (
                                <div key={`${student.id}-${student._lastUpdate || 'no-update'}`} onClick={() => setPageState({ page: 'profile', userId: student.id})} className="text-center cursor-pointer group">
                                    <p className="font-semibold text-gray-800 group-hover:text-cornell-red transition">{student.firstName} {student.lastName}</p>
                                </div>
                            ))}
                        </div>
                    ) : (
                  ...
                  <div className="space-y-6">
                    ...
                    {signedUpStudents.length > 0 && (
                      <div className="bg-white p-6 rounded-2xl shadow-lg">
                        ...
                        <button
                          onClick={() => {
                            // Get all participant emails by matching involved_users with full student data
                            const participantEmails = signedUpStudents
                              .map(involvedUser => {
                                // Find the full student data from the students array
                                const fullStudent = students.find(student => student.id === involvedUser.id);
                                return fullStudent?.email;
                              })
                              .filter(email => email) // Only include students with emails
                              .join(', ');
                            ...
                            if (participantEmails) {
                              ...
                              alert('No participant emails available to send to. This might be because participant emails are not stored in the system.');
                            }
                          }}
