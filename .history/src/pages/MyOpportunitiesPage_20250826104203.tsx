...
const MyOpportunitiesPage: React.FC<MyOpportunitiesPageProps> = ({ 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            {hostedOpportunities.map(opp => (
              <OpportunityCard
                key={opp.id}
                opportunity={opp}
                onViewDetails={() => setPageState({ page: 'opportunityDetail', id: opp.id })}
                isHost={true}
              />
            ))}
          </div>
      ...
      <div className="mb-12">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-gray-900">Opportunities I'm Registered For</h2>
        ...
        {registeredOpportunities.length === 0 && !showPastRegistered && (
          <div className="text-center py-8 text-gray-500">
            <p>You're not registered for any upcoming opportunities.</p>
          </div>
        )}
