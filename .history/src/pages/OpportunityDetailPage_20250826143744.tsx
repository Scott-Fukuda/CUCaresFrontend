...
const OpportunityDetailPage: React.FC<OpportunityDetailPageProps> = ({ opportunity, students, signups, currentUser, handleSignUp, handleUnSignUp, setPageState, allOrgs, currentUserSignupsSet }) => {
  ...
  const displayDate = currentOpportunity.date ? new Date(currentOpportunity.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 'TBD';
  
  // Parse time correctly for display (assuming Eastern Time)
  const displayTime = currentOpportunity.time ? (() => {
    const [hours, minutes] = currentOpportunity.time.split(':');
    return new Date(2024, 0, 1, parseInt(hours), parseInt(minutes)).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
  })() : 'TBD';
