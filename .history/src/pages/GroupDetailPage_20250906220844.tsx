        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg">
            ...
            {currentUser.admin && (
                <div className="mb-4 p-2 bg-purple-50 border border-purple-200 rounded-lg">
                    <p className="text-sm text-purple-700 font-medium">Admin View - Organization ID: {org.id}</p>
                </div>
            )}
            <button
                        ...
                        <li key={event.id} onClick={() => setPageState({ page: 'opportunityDetail', id: event.id })} className="p-4 rounded-lg bg-light-gray flex items-center justify-between cursor-pointer hover:bg-gray-200 transition-colors">
                            <div>
                                ...
                                <p className="text-sm text-gray-600">
                                    {new Date(event.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} &bull; {event.nonprofit}
                                </p>
                            </div>
