  ...
  const renderPage = () => {
    ...
    switch (pageState.page) {
        ...
        case 'profile':
            ...
            // If user not found in students array, try to find them in other data sources
            if (!profileUser && pageState.userId) {
                // Check in opportunities' involved_users
                for (const opp of opportunities) {
                    if (opp.involved_users) {
                        const foundUser = opp.involved_users.find(u => u.id === pageState.userId);
                        if (foundUser) {
  ...
  return (
    <div className="min-h-screen bg-light-gray pb-20 md:pb-0">
      ...
      <a
        ...
        className="fixed top-4 right-4 md:bottom-6 md:top-auto md:right-6 bg-red-600 hover:bg-red-700 text-white px-3 py-2 md:px-4 md:py-2 rounded-full shadow-lg transition-colors duration-200 flex items-center gap-1 md:gap-2 z-50 text-sm md:text-base"
        title="Report a Bug"
      >
        <svg 
