...
export const getAcceptedFriendships = async (userId: number): Promise<User[]> => {
  ...
  try {
    const result = await request(`/users/${userId}/accepted-friendships`);
    console.log(`API: Accepted friendships response:`, result);
    
    // The backend returns { accepted_friendships: [...], count: number }
    if (result && Array.isArray(result.accepted_friendships)) {
      // Transform the friendship objects to User objects
      return result.accepted_friendships.map((friendship: any) => ({
        id: friendship.other_user_id,
        firstName: friendship.other_user_name.split(' ')[0] || '',
        lastName: friendship.other_user_name.split(' ').slice(1).join(' ') || '',
        email: '', // Not provided in friendship response
        password: '', // Not provided in friendship response
        profilePictureUrl: friendship.other_user_profile_image,
    ...
    console.warn('Unexpected accepted friendships response structure:', result);
    return [];
  } catch (error) {
