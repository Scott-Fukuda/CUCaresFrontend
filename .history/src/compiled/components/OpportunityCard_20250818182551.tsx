...
const OpportunityCard: React.FC<OpportunityCardProps> = ({ opportunity, signedUpStudents, allGroups, currentUser, onSignUp, onUnSignUp, isUserSignedUp, setPageState }) => {
  ...
  // Use involved_users from backend if available, otherwise fall back to signedUpStudents
  const participants = opportunity.involved_users || signedUpStudents;
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex justify-between items-center text-sm font-medium text-gray-600 mb-2">
            <div className="flex items-center gap-2">
                ...
                <span>Signed Up ({participantCount}/{opportunity.totalSlots})</span>
            </div>
        ...
        {participantCount > 0 ? (
             <div className="flex items-center -space-x-2 mb-4">
                {participants.slice(0, 10).map(student => (
                ...
                {participantCount > 10 && <div className="h-8 w-8 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center text-xs font-semibold ring-2 ring-white">+{participantCount - 10}</div>}
             </div>
        ) : (
        ...
        <button
          ...
          disabled={(!canSignUp && !isUserSignedUp) || isUserHost}
        ...
        >
          {isUserHost ? 'You are the Host' : isUserSignedUp ? 'Signed Up ✓' : canSignUp ? 'Sign Up' : 'No Slots Available'}
        </button>
      </div>
    </div>
  );
