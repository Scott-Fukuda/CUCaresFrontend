...
export const getOpportunities = async (): Promise<Opportunity[]> => {
    try {
        ...
        const transformedOpportunities = (response.opportunities || []).map((opp: any) => {
            ...
            console.log('Raw opportunity involved_users:', opp.involved_users);
            const transformedInvolvedUsers = (opp.involved_users || []).map((involvedUser: any) => {
                ...
                console.log('Raw involved user user field:', involvedUser.user);
            ...
            const registeredUsers = transformedInvolvedUsers.filter(user => user.registered === true);
...
export const unregisterForOpp = (data: { user_id: number; opportunity_id: number }) => authenticatedRequest('/unregister-opp', {
  method: 'POST',
  body: JSON.stringify(data),
});
