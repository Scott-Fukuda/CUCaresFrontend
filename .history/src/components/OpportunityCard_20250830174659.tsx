  ...
  return (
    <div onClick={handleCardClick} className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col transition-transform hover:scale-105 duration-300 cursor-pointer">
      ...
      <div className="p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-2">
            <div className="flex items-center gap-2">
                ...
                {opportunity.host_org_name && (
                    <span className="text-sm font-semibold text-blue-600 uppercase tracking-wider">Hosted by {opportunity.host_org_name}</span>
                )}
                {isUserHost && (
        ...
        <p className="text-gray-500 text-sm mb-4">{displayDate} &bull; {displayTime} &bull; {opportunity.duration} min</p>
        {opportunity.causes && opportunity.causes.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-4">
            {opportunity.causes.map((cause, index) => (
              <span key={index} className="text-xs bg-cornell-red/10 text-cornell-red px-2 py-1 rounded-full">
                {cause}
              </span>
            ))}
          </div>
