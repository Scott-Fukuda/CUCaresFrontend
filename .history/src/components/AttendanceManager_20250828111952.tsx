...
const AttendanceManager: React.FC<AttendanceManagerProps> = ({ opportunity, participants, onAttendanceSubmitted }) => {
  ...
  useEffect(() => {
    const checkTime = () => {
      ...
      // Create the opportunity date/time string in Eastern Time
      ...
      // Create a date object representing the opportunity time in Eastern Time
      // We'll use the offset approach to ensure Eastern Time comparison
      const opportunityDateTime = new Date(opportunityDateString + '-05:00'); // EST offset
      
      // Get current time in Eastern Time by using the offset
      const nowEastern = new Date(now.toLocaleString("en-US", {timeZone: "America/New_York"}));
      
      console.log('Current time (Eastern):', nowEastern);
      console.log('Event start time (Eastern):', opportunityDateTime);
      console.log('Raw opportunity date string:', opportunityDateString);
      console.log('Attendance enabled:', nowEastern >= opportunityDateTime);
      
      setIsSubmitEnabled(nowEastern >= opportunityDateTime);
    };
