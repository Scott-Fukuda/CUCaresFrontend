...
const AttendanceManager: React.FC<AttendanceManagerProps> = ({ opportunity, participants, onAttendanceSubmitted }) => {
  ...
  useEffect(() => {
    const checkTime = () => {
      ...
      // Create opportunity date/time in Eastern Time
      ...
      const easternTime = new Date(opportunityDateTime.toLocaleString("en-US", {timeZone: "America/New_York"}));
      const nowEastern = new Date(now.toLocaleString("en-US", {timeZone: "America/New_York"}));
      
      console.log('Current time (Eastern):', nowEastern);
      console.log('Event start time (Eastern):', easternTime);
      console.log('Attendance enabled:', nowEastern >= easternTime);
      
      setIsSubmitEnabled(nowEastern >= easternTime);
    };
