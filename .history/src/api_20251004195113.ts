...
// --- Create Opportunity ---
// Accept either a FormData (for multipart uploads) or a plain object (JSON payload)
export const createOpportunity = async (data: FormData | object): Promise<Opportunity> => {
  const token = await getFirebaseToken();
  const headers: Record<string, string> = {};
  if (token) {
    headers['Authorization'] = `Bearer ${token}`;
  ...
  const response = await fetch(`${ENDPOINT_URL}/api/opps`, fetchOptions);
