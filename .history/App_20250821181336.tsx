...
const App: React.FC = () => {
  ...
  const getFriendsForUser = useCallback((userId: number): User[] => {
    console.log('Getting friends for user:', userId);
    console.log('Current friendships array:', friendships);
    console.log('Available students:', students.map(s => ({ id: s.id, name: `${s.firstName} ${s.lastName}` })));
    
    if (!Array.isArray(friendships)) {
      console.log('Friendships is not an array, returning empty');
      return [];
    }
    
    const userFriendships = friendships.filter(f => f.status === 'accepted' && (f.user1_id === userId || f.user2_id === userId));
    console.log('Accepted friendships for user:', userFriendships);
    
    const friendUsers = userFriendships
      .map(f => {
        const friendId = f.user1_id === userId ? f.user2_id : f.user1_id;
        const friendUser = students.find(s => s.id === friendId);
        console.log(`Mapping friendship: friendId=${friendId}, found user:`, friendUser ? `${friendUser.firstName} ${friendUser.lastName}` : 'not found');
        return friendUser;
      })
      .filter((user): user is User => user !== undefined);
