...
const OpportunitiesPage: React.FC<OpportunitiesPageProps> = ({ opportunities, students, signups, currentUser, handleSignUp, handleUnSignUp, setPageState, allOrgs, currentUserSignupsSet }) => {
  ...
  // Debug logging
  console.log('OpportunitiesPage render:', {
    ...
    const filtered = opportunities
      .filter(opp => {
        ...
        if (causeFilter !== 'All' && opp.cause !== causeFilter) {
          console.log(`Opportunity ${opp.id} filtered out by cause: ${opp.cause} !== ${causeFilter}`);
          return false;
        }
        ...
        if (dateFilter) {
            ...
            if (oppDate.getTime() !== filterDate.getTime()) {
                console.log(`Opportunity ${opp.id} filtered out by date: ${oppDate.toISOString()} !== ${filterDate.toISOString()}`);
                return false;
            }
        }
        ...
        console.log(`Opportunity ${opp.id} (${opp.name}):`, {
          oppDate: oppDate.toISOString(),
          today: today.toISOString(),
          isFuture,
          cause: opp.cause,
          causeFilter,
          matchesCause: causeFilter === 'All' || opp.cause === causeFilter
        });
        return isFuture;
      })
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredOpportunities.map(opp => {
          ...
          const isUserSignedUp = opp.involved_users 
            ? opp.involved_users.some(user => user.id === currentUser.id && user.registered === true)
