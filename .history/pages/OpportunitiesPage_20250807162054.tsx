...
const OpportunitiesPage: React.FC<OpportunitiesPageProps> = ({ opportunities, students, signups, currentUser, handleSignUp, handleUnSignUp, setPageState, allOrgs, currentUserSignupsSet }) => {
  ...
  const filteredOpportunities = useMemo(() => {
    ...
    console.log('Debug - Total opportunities:', opportunities.length);
    ...
    const filtered = opportunities
      .filter(opp => {
        ...
        console.log('Debug - Opportunity date:', opp.date, 'Parsed as:', oppDate.toISOString());
        ...
        // Don't show past events - temporarily disabled for debugging
        const isFutureOrToday = oppDate.getTime() >= today.getTime();
        console.log('Debug - Is future or today:', isFutureOrToday);
        // Temporarily show all events for debugging
        return true; // oppDate.getTime() >= today.getTime();
      })
      .sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    
    console.log('Debug - Filtered opportunities:', filtered.length);
    return filtered;
