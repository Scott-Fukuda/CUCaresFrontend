...
const OpportunitiesPage: React.FC<OpportunitiesPageProps> = ({ opportunities, students, signups, currentUser, handleSignUp, handleUnSignUp, setPageState, allOrgs, currentUserSignupsSet }) => {
  ...
  const filteredOpportunities = useMemo(() => {
    ...
    console.log('Debug - Total opportunities received:', opportunities.length);
    console.log('Debug - Today\'s date:', today.toISOString());
    console.log('Debug - All opportunities:', opportunities);
    
    const filtered = opportunities
      .filter(opp => {
        // Cause filter
        if (causeFilter !== 'All' && opp.cause !== causeFilter) {
          console.log('Debug - Filtered out by cause:', opp.title, 'cause:', opp.cause, 'filter:', causeFilter);
          return false;
        }
        
        // Parse the opportunity date string as a local date by adding T00:00:00
        const oppDate = new Date(`${opp.date}T00:00:00`);
        console.log('Debug - Opportunity:', opp.title, 'date:', opp.date, 'parsed:', oppDate.toISOString());
        ...
        if (dateFilter) {
            ...
            if (oppDate.getTime() !== filterDate.getTime()) {
                console.log('Debug - Filtered out by date filter:', opp.title);
                return false;
            }
        }

        // Don't show past events
        const isFutureOrToday = oppDate.getTime() >= today.getTime();
        console.log('Debug - Is future or today:', isFutureOrToday, 'for:', opp.title);
        return isFutureOrToday;
      })
      .sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    
    console.log('Debug - Final filtered opportunities:', filtered.length);
    return filtered;
