...
const OpportunitiesPage: React.FC<OpportunitiesPageProps> = ({ opportunities, students, signups, currentUser, handleSignUp, handleUnSignUp, setPageState, allOrgs, currentUserSignupsSet }) => {
  ...
  console.log('OpportunitiesPage received:', {
  ...
  const filteredOpportunities = useMemo(() => {
    ...
    console.log('Filtering opportunities. Today:', today.toISOString());

    return opportunities
      .filter(opp => {
        console.log('Checking opportunity:', opp.id, opp.name, 'date:', opp.date);
        
        // Cause filter
        if (causeFilter !== 'All' && opp.cause !== causeFilter) {
          console.log('Filtered out by cause:', opp.cause, '!=', causeFilter);
          return false;
        }
        
        // Parse the opportunity date string as a local date by adding T00:00:00
        const oppDate = new Date(`${opp.date}T00:00:00`);
        console.log('Parsed opportunity date:', oppDate.toISOString());
        ...
        if (dateFilter) {
            ...
            if (oppDate.getTime() !== filterDate.getTime()) {
                console.log('Filtered out by date filter');
                return false;
            }
        }

        // Don't show past events
        const isFuture = oppDate.getTime() >= today.getTime();
        console.log('Is future event:', isFuture, 'oppDate:', oppDate.getTime(), 'today:', today.getTime());
        return isFuture;
      })
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredOpportunities.map(opp => {
          // Use involved_users from backend if available, otherwise fall back to local signups
          const signedUpStudents = opp.involved_users || students.filter(student =>
