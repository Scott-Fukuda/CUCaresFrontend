...
export const getUserFriends = async (userId: number): Promise<User[]> => {
  ...
  if (result && Array.isArray(result.friendships)) {
    // Filter only accepted friendships and transform to User objects
    const acceptedFriendships = result.friendships.filter((f: any) => f.accepted === true);
    console.log('Accepted friendships:', acceptedFriendships);
    
    // For now, we'll return an empty array since we need to get full user data
    // The actual user data should come from the main users list
    return [];
  }
...
export const acceptFriendRequest = (friendshipId: number) => request(`/friendships/${friendshipId}/accept`, {
  method: 'PUT',
});

export const rejectFriendRequest = (friendshipId: number) => request(`/friendships/${friendshipId}/reject`, {
  method: 'PUT',
});
