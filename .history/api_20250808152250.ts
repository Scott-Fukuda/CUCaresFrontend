...
export const getOpportunities = async (): Promise<Opportunity[]> => {
    try {
        ...
        const transformedOpportunities = (response.opportunities || []).map((opp: any) => ({
            id: opp.id,
            title: opp.name, // Backend uses 'name', frontend expects 'title'
            organization: `Organization ${opp.host_org_id}`, // Use host_org_id as placeholder
            description: opp.description,
            date: new Date(opp.date).toISOString().split('T')[0], // Convert to YYYY-MM-DD format
            time: '14:00', // Default time since backend doesn't provide it
            duration: opp.duration,
            totalSlots: 10, // Default since backend doesn't provide it
            imageUrl: 'https://via.placeholder.com/300x200?text=Opportunity', // Default image
