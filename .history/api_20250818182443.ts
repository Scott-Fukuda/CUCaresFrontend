...
export const getOpportunities = async (): Promise<Opportunity[]> => {
    try {
        ...
        const transformedOpportunities = (response.opportunities || []).map((opp: any) => {
            ...
            // If the original string contains GMT, use UTC methods to preserve the original time
            // Otherwise, assume Eastern Time (UTC-4) and convert accordingly
            let timeOnly;
            if (opp.date.includes('GMT')) {
                // GMT format - use UTC methods to preserve original time
                const hours = dateObj.getUTCHours().toString().padStart(2, '0');
                const minutes = dateObj.getUTCMinutes().toString().padStart(2, '0');
                const seconds = dateObj.getUTCSeconds().toString().padStart(2, '0');
                timeOnly = `${hours}:${minutes}:${seconds}`;
            } else {
                // Assume Eastern Time (UTC-4) - convert to UTC by adding 4 hours
                const easternHours = dateObj.getHours();
                const utcHours = (easternHours + 4) % 24; // Convert Eastern to UTC
