...
interface RegisterProps {
  onRegister: (firstName: string, lastName: string, phone: string, gender: string, graduationYear: string, academicLevel: string, major: string, birthday: string) => void;
  onBackToLogin: () => void;
  error: string | null;
  isLoading: boolean;
}
...
const Register: React.FC<RegisterProps> = ({ onRegister, onBackToLogin, error, isLoading }) => {
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (firstName.trim() && lastName.trim() && phone.trim() && graduationYear.trim() && academicLevel.trim() && major.trim() && birthday.trim()) {
      onRegister(firstName.trim(), lastName.trim(), phone.trim(), gender, graduationYear.trim(), academicLevel.trim(), major.trim(), birthday.trim());
    }
  };
