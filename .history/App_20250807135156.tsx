...
import { signInWithGoogle, FirebaseUser } from './firebase-config';
...
const App: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Handle different response formats based on your backend
      if (response.ok && responseData.status === 'authenticated') {
        ...
        if (responseData.error === 'Authorization header is required') {
          setAuthError('Authentication failed: Please provide a valid Firebase ID token');
        } else if (responseData.error === 'Invalid token') {
          setAuthError('Authentication failed: Token is invalid or expired');
        } else {
          setAuthError(responseData.message || 'Authentication failed. Please try again.');
        }
      }
