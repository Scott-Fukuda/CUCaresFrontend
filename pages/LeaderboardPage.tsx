...
import { User, Organization, SignUp, Opportunity, OrganizationCategory, organizationCategories } from '../types';
...
type LeaderboardTab = 'Individuals' | 'All Organizations' | OrganizationCategory;

const TABS: LeaderboardTab[] = ['Individuals', 'All Organizations', ...organizationCategories];

const LeaderboardPage: React.FC<LeaderboardPageProps> = ({ allUsers, allOrgs, signups, opportunities, currentUser, handleFriendRequest, setPageState }) => {
  ...
  const groupLeaderboard = useMemo(() => {
    const filteredOrgs = activeTab === 'All Organizations' 
        ? allOrgs 
        : allOrgs.filter(g => g.category === activeTab);
               ...
               <li key={org.id} className="flex items-center justify-between py-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="cursor-pointer" onClick={() => setPageState({ page: 'groupDetail', id: org.id})}>
                    ...
                    <p className="text-sm text-gray-500">{memberCount} members &bull; {org.category}</p>
                  </div>
                </div>
                <span className="font-bold text-cornell-red text-lg">{points} pts</span>
              </li>
